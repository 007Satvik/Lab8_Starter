describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    var val = await innertext.jsonValue();
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for(let i = 1; i < 20; i++){
      ...
      var but = await shadowRoot.$('button');
      await but.click(); 
    }

    const cartCount = await page.$("#cart-count");
    const val = await cartCount.getProperty('innerText')
    // get the shadowRoot and query select the button inside, and click on it.
    // Check to see if the innerText of #cart-count is 20
    const count = await val.jsonValue();
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for(let i = 0; i < 20; i++){
      ...
      const prop = await innerText.jsonValue();
      if(prop == 'Add to Cart'){check = false;}
    }
    const cartCount = await page.$("#cart-count");
    const val = await cartCount.getProperty('innerText');
    const count = await val.jsonValue();
